...
const VERSION = 0.6
console.log('worktipsnet launcher version', VERSION, 'registered')
...
function testDNSForWorktipsnet(server, cb) {
  const resolver = new dns.Resolver()
  resolver.setServers([server])
  // incase server is 127.0.0.1:undefined
  try {
    resolver.resolve('localhost.worktips', function (err, records) {
      if (err) console.error('NETWORK: localhost.worktips resolve err:', err)
      //log(server, 'localhost.worktips test results', records)
      cb(records)
    })
  } catch (e) {
    console.error('NETWORK: testDNSForWorktipsnet error, incorrect server?', server)
    cb()
  }
}
...
function lookup(host, cb) {
  ...
  //console.log('worktipsnet lookup servers', runningConfig.dns.bind)
    ...
    if (err) {
      ...
      if (err.code == 'ENOTFOUND') {
        ...
        if (err.code == 'ETIMEOUT') {
          ...
          console.error('worktipsnet lookup unknown err', err)
        }
    }
    //console.log(host, 'worktipsnet dns test results', records)
    cb(records)
  })
}

function findWorktipsNetDNS(cb) {
  ...
  function checkDone() {
    if (shuttingDown) {
      //if (cb) cb()
      log('not going to start worktipsnet, shutting down')
      return
    }
    checksLeft--
    if (checksLeft <= 0) {
  ...
  for(var i in resolvers) {
    ...
    if (idx != -1) {
      ...
      checksLeft++
      testDNSForWorktipsnet(server, function(isWorktipsnet) {
        if (isWorktipsnet) {
          // worktipsnet
          console.log(server, 'is a worktipsnet DNS server')
          servers.push(server)
        }
        checkDone()
      })
  ...
  */
  ...
  for (var i in localIPs) {
    ...
    checksLeft++
    testDNSForWorktipsnet(server, function (isWorktipsnet) {
      if (isWorktipsnet !== undefined) {
        // worktipsnet
        log(server, 'is a worktipsnet DNS server')
        servers.push(server)
      }
      checkDone()
    })
  ...
  for (var i in resolvers) {
    ...
    if (idx != -1) {
      ...
      testDNSForWorktipsnet(server, function (isWorktipsnet) {
        if (isWorktipsnet === undefined) {
          // not worktipsnet
          log(server, 'is not a worktipsnet DNS server')
          servers.push(server)
        }
        checkDone()
      })
      ...
      if (idx != -1) {
        ...
        resolver.resolve('localhost.worktips', function(err, records) {
          ...
          if (records === undefined) {
            // not worktipsnet
            console.log(server, 'is not a worktipsnet DNS server')
            servers.push(server)
          }
          checkDone()
        })
...
// this can really delay the start of worktipsnet
...
function generateINI(config, need, markDone, cb) {
  ...
  //const data = fs.readFileSync(homeDir + '/.worktipsnet/worktipsnet.ini', 'utf-8')
  //const jConfig = iniToJSON(data)
  //console.dir(jConfig)
  //const iConfig = jsonToINI(jConfig)
  //console.log(iConfig)
  var upstreams, worktipsnet_free53Ip, worktipsnet_nic
  var use_worktipsnet_rpc_port = config.rpc_port
  var worktipsnet_bootstrap_path = homeDir + '/.worktipsnet/bootstrap.signed'
  var worktipsnet_nodedb = homeDir + '/.worktipsnet/netdb'
  if (config.netid) {
    worktipsnet_nodedb += '-' + config.netid
  }
  if (!fs.existsSync(worktipsnet_nodedb)) {
    log('making', worktipsnet_nodedb)
    mkDirByPathSync(worktipsnet_nodedb)
  }
  var upstreamDNS_servers = []
  var params = {
    upstreamDNS_servers: upstreamDNS_servers,
    worktipsnet_free53Ip: worktipsnet_free53Ip,
    worktipsnet_nodedb: worktipsnet_nodedb,
    worktipsnet_bootstrap_path: worktipsnet_bootstrap_path,
    worktipsnet_nic: worktipsnet_nic,
    use_worktipsnet_rpc_port: use_worktipsnet_rpc_port,
  }
  if (config.bootstrap_url) {
    httpGet(config.bootstrap_url, function (bootstrapData) {
      if (bootstrapData) {
        cleanUpBootstrap = true
        const tmpRcPath = os.tmpdir() + '/' + randomString(8) + '.worktipsnet_signed'
        fs.writeFileSync(tmpRcPath, bootstrapData, 'binary')
        log('boostrap wrote', bootstrapData.length, 'bytes to', tmpRcPath)
        //worktipsnet_bootstrap_path = tmpRcPath
        params.worktipsnet_bootstrap_path = tmpRcPath
    ...
    // seed version
    //params.worktipsnet_bootstrap_path = ''
    markDone('bootstrap', params)
  }
  ...
  httpGet('http://' + config.rpc_ip + ':' + config.rpc_port, function (testData) {
    ...
    if (testData !== undefined) {
      ...
      use_worktipsnet_rpc_port = use_worktipsnet_rpc_port + 1
      params.use_worktipsnet_rpc_port = use_worktipsnet_rpc_port
    }
  ...
  getNetworkIP(function (e, ip) {
    ...
    worktipsnet_nic = getIfNameFromIP(ip)
    params.worktipsnet_nic = worktipsnet_nic
    params.interfaceIP = ip
    log('detected outgoing interface', worktipsnet_nic)
    ...
    findFreePort53(tryIps, 0, function (free53Ip) {
      if (free53Ip === undefined) {
        console.error('NETWORK: Cant automatically find an IP to put a worktipsnet DNS server on')
        // can't handle the exits here because we don't know if it's an actual requirements
        if (done.dnsBind !== undefined) {
          process.exit()
        }
      }
      worktipsnet_free53Ip = free53Ip
      params.worktipsnet_free53Ip = free53Ip
...
function applyConfig(file_config, config_obj) {
  ...
  if (file_config.worktipsd && file_config.worktipsd.network == "test") {
...
function generateSerivceNodeINI(config, cb) {
  ...
  function markDone(completeProcess, params) {
    ...
    var keyPath = homeDir + '/.worktips/'
    //
    if (config.worktipsd.data_dir) {
      keyPath = config.worktipsd.data_dir
      // make sure it has a trailing slash
      if (keyPath[keyPath.length - 1] != '/') {
        keyPath += '/'
      }
    }
    if (config.worktipsd.network == "test" || config.worktipsd.network == "demo") {
      keyPath += 'testnet/'
    }
    keyPath += 'key'
    ...
    log('Drafting worktipsnet service node config')
    ...
    runningConfig = {
      ...
      dns: {
        ...
        bind: params.worktipsnet_free53Ip + ':53',
      },
      netdb: {
        dir: params.worktipsnet_nodedb,
      },
      bind: {
        // will be set after
      },
      ...
      api: {
        enabled: true,
        bind: config.rpc_ip + ':' + params.use_worktipsnet_rpc_port
      },
      worktipsd: {
        enabled: true,
        jsonrpc: config.worktipsd.rpc_ip + ':' + config.worktipsd.rpc_port,
        username: config.worktipsd.rpc_user,
        password: config.worktipsd.rpc_pass,
    ...
    runningConfig.bind[params.worktipsnet_nic] = config.public_port
    if (config.internal_port) {
      runningConfig.bind[params.worktipsnet_nic] = config.internal_port
    }
...
function generateClientINI(config, cb) {
  ...
  function markDone(completeProcess, params) {
    ...
    if (!params.use_worktipsnet_rpc_port) {
      // use default because we enable it
      params.use_worktipsnet_rpc_port = 1190
    }
    log('Drafting worktipsnet client config')
    runningConfig = {
      router: {
        nickname: 'ldl',
      },
...
var shuttingDown
var worktipsnet
var worktipsnetLogging = true

function preLaunchWorktipsnet(config, cb) {
  ...
  if (os.platform() == 'linux') {
    ...
    exec('getcap ' + config.binary_path, function (error, stdout, stderr) {
      ...
      // src/worktips-network/worktipsnet = cap_net_bind_service,cap_net_admin+eip
  ...
  // worktipsnet will crash if this file is zero bytes
  ...
  const tmpPath = tmpDir + '/' + randomString(8) + '.worktipsnet_ini'
  cleanUpIni = true
  config.ini_writer(config, function (iniData) {
    if (shuttingDown) {
      //if (cb) cb()
      log('not going to write worktipsnet config, shutting down')
      return
    }
...
function launchWorktipsnet(config, cb) {
  if (shuttingDown) {
    //if (cb) cb()
    log('not going to start worktipsnet, shutting down')
    return
  ...
  if (!fs.existsSync(config.ini_file)) {
    log('worktipsnet config file', config.ini_file, 'does not exist')
    process.exit()
  }
  ...
  worktipsnet = spawn(config.binary_path, cli_options)

  if (!worktipsnet) {
    console.error('failed to start worktipsnet, exiting...')
    // proper shutdown?
    process.exit()
  }
  worktipsnet.killed = false
  worktipsnet.stdout.on('data', (data) => {
  ...
  worktipsnet.stderr.on('data', (data) => {
    if (module.exports.onError) {
      module.exports.onError(data)
    }
  })

  worktipsnet.on('close', (code) => {
    log(`worktipsnet process exited with code ${code}`)
    // code 0 means clean shutdown
    worktipsnet.killed = true
    // clean up
    ...
    if (!shuttingDown) {
      if (config.restart) {
        ...
        setTimeout(function () {
          log('worktips_daemon is still running, restarting worktipsnet')
          launchWorktipsnet(config)
        }, 30 * 1000)
      } else {
        // don't restart...
      }
...
function checkConfig(config) {
  ...
  if (config.binary_path === undefined) config.binary_path = '/usr/local/bin/worktipsnet'
...
function startServiceNode(config, cb) {
  ...
  // only us worktipsnet devs will need to make our own seed node
  preLaunchWorktipsnet(config, function () {
    // test worktipsd rpc port first
    // also this makes sure the service key file exists
    var url = 'http://' + config.worktipsd.rpc_user + ':' + config.worktipsd.rpc_pass + '@' + config.worktipsd.rpc_ip + ':' + config.worktipsd.rpc_port
    log('worktipsnet waiting for worktipsd RPC server')
    waitForUrl(url, function () {
      launchWorktipsnet(config, cb)
    })
  })
}
...
// intent to stop worktipsnet and don't restart it
var retries = 0

function stop() {
  shuttingDown = true
  if (worktipsnet && worktipsnet.killed) {
    console.warn('worktipsnet already stopped')
    retries++
    if (retries > 3) {
  ...
  log('requesting worktipsnet be shutdown')
  if (worktipsnet && !worktipsnet.killed) {
    log('sending SIGINT to worktipsnet', worktipsnet.pid)
    process.kill(worktipsnet.pid, 'SIGINT')
    worktipsnet.killed = true
    // HACK: worktipsnet on macos can not be killed if rpc port is in use
    var monitorTimerStart = Date.now()
    var monitorTimer = setInterval(function () {
      if (!isPidRunning(worktipsnet.pid)) {
        // launcher can't exit until this interval is cleared
        clearInterval(monitorTimer)
      } else {
        var diff = Date.now() - monitorTimerStart
        if (diff > 15 * 1000) {
          // reach 15 secs and worktipsnet is still running
          // escalate it
          console.error('Worktipsnet is still running 15s after we intentionally stopped worktipsnet?')
          process.kill(worktipsnet.pid, 'SIGKILL')
        } else
        if (diff > 30 * 1000) {
          // reach 30 secs and worktipsnet is still running
          // escalate it
          console.error('Worktipsnet is still running 30s after we intentionally killed worktipsnet?')
    ...
    setTimeout(function() {
      try {
        ...
        process.kill(worktipsnet.pid, 0)
        log('sending SIGKILL to worktipsnet')
        process.kill(worktipsnet.pid, 'SIGKILL')
      } catch(e) {
        console.error('Launcher is still running 15s after we intentionally stopped worktipsnet?')
        var handles = process._getActiveHandles()
        console.log('handles', handles.length)
        for(var i in handles) {
          var handle = handles[i]
...
function getPID() {
  return (worktipsnet && !worktipsnet.killed && worktipsnet.pid) ? worktipsnet.pid : 0
}

function enableLogging() {
  worktipsnetLogging = true
}

function disableLogging() {
  console.log('Disabling worktipsnet logging')
  worktipsnetLogging = false
}

function getWorktipsNetIP(cb) {
  function checkDNS() {
    log('worktipsnet seems to be running, querying', runningConfig.dns.bind)
    // where's our DNS server?
    //log('RunningConfig says our worktipsnet\'s DNS is on', runningConfig.dns.bind)
    testDNSForWorktipsnet(runningConfig.dns.bind, function (ips) {
      //log('worktipsnet test', ips)
      if (ips && ips.length) {
        cb(ips[0])
      } else {
        // , retrying
        console.error('cant communicate with worktipsnet DNS')
        /*
        //process.exit()
        setTimeout(function() {
          getWorktipsNetIP(cb)
  ...
  if (runningConfig.api.enabled) {
    log('wait for worktipsnet startup', runningConfig.api)
...
module.exports = {
  ...
  findWorktipsNetDNS: findWorktipsNetDNS,
  lookup: lookup,
  isRunning: isRunning,
  stop: stop,
  getWorktipsNetIP: getWorktipsNetIP,
  enableLogging: enableLogging,
  disableLogging: disableLogging,
  getPID: getPID,
  // FIXME: should we allow hooking of log() too?
  onMessage: function (data) {
    if (worktipsnetLogging) {
      console.log(`worktipsnet: ${data}`)
    }
  },
  onError: function (data) {
    console.log(`worktipsneterr: ${data}`)
  },
}
