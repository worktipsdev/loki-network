...
namespace llarp
{
  ...
  Router::FromConfig(Config *conf)
  {
    ...
    // Worktipsd Config
    usingSNSeed      = conf->worktipsd.usingSNSeed;
    ident_keyfile    = conf->worktipsd.ident_keyfile;
    whitelistRouters = conf->worktipsd.whitelistRouters;
    worktipsdRPCAddr     = conf->worktipsd.worktipsdRPCAddr;
    worktipsdRPCUser     = conf->worktipsd.worktipsdRPCUser;
    worktipsdRPCPassword = conf->worktipsd.worktipsdRPCPassword;
  ...
  Router::Run()
  {
    ...
    if(whitelistRouters)
    {
      ...
      rpcCaller->SetAuth(worktipsdRPCUser, worktipsdRPCPassword);
      while(!rpcCaller->Start(worktipsdRPCAddr))
      {
        LogError("failed to start jsonrpc caller to ", worktipsdRPCAddr);
#if defined(ANDROID) || defined(RPI)
        sleep(1);
#else
        std::this_thread::sleep_for(std::chrono::seconds(1));
      ...
      LogInfo("RPC Caller to ", worktipsdRPCAddr, " started");
    }
